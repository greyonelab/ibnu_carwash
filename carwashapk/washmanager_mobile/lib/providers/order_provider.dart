...
class OrderProvider with ChangeNotifier {
  ...
  List<WashLane> get washLanes => _washLanes;
  ...
  Future<void> loadWashLanes() async {
    print('🔄 OrderProvider: Loading wash lanes...');
    try {
      _washLanes = await _apiService.getWashLanes();
      print('✅ OrderProvider: Loaded ${_washLanes.length} wash lanes');
      _washLanes.forEach((lane) {
        print('   - ${lane.name} (${lane.type}): ${lane.currentQueue}/${lane.maxQueue}');
      });
      notifyListeners();
    } catch (e) {
      _errorMessage = 'Failed to load wash lanes: $e';
      print('❌ OrderProvider: Error loading wash lanes - $e');
      notifyListeners();
    }
  }
  ...
  Future<WashLane?> getAvailableLane(String vehicleType) async {
    try {
      return await _apiService.getAvailableLane(vehicleType);
    } catch (e) {
      print('❌ OrderProvider: Error getting available lane - $e');
      return null;
    }
  }
